...
(def test-6x5-empty-board {:width 6 :height 5 :board {} :game-log []})
(def test-6x5-laid-board  {:width 6 :height 5 :board {0 :red 2 :yellow 5 :red 6 :yellow 7 :yellow} :game-log []})
(def test-4x5-laid-board  {:width 4 :height 5 :board {0 :red 2 :yellow 5 :red 6 :yellow 7 :yellow} :game-log []})
(def test-4x3-laid-board  {:width 4 :height 3 :board {0 :red 4 :yellow 8 :red} :game-log []})

(sut/print-board test-4x3-laid-board)

(deftest test-board-creation
  (testing "default num board is empty regardless of size"
    (is (= {:width 1 :height 1 :board {} :game-log []} (sut/create-empty-board 1 1)))
    (is (= {:width 1 :height 2 :board {} :game-log []} (sut/create-empty-board 1 2)))
    (is (= {:width 1 :height 3 :board {} :game-log []} (sut/create-empty-board 1 3)))
    (is (= {:width 2 :height 1 :board {} :game-log []} (sut/create-empty-board 2 1)))
    (is (= {:width 3 :height 1 :board {} :game-log []} (sut/create-empty-board 3 1)))
    (is (= {:width 2 :height 2 :board {} :game-log []} (sut/create-empty-board 2 2)))
  )
  )
...
(deftest test-place-counter
  (testing "Places in the location specified on empty board"
    (is (= {:width 6 :height 5 :board {0 :red} :game-log []} (sut/place-counter test-6x5-empty-board 0 0 :red)))
    (is (= {:width 6 :height 5 :board {1 :yellow} :game-log []} (sut/place-counter test-6x5-empty-board 1 0 :yellow)))
    (is (= {:width 6 :height 5 :board {6 :yellow} :game-log []} (sut/place-counter test-6x5-empty-board 0 1 :yellow)))
  )
  (testing "Places in the location specified on non-empty board"
    (is (= {:width 4 :height 3 :board {0 :red 4 :yellow 8 :red 5 :yellow} :game-log []} (sut/place-counter test-4x3-laid-board 1 1 :yellow)))
    (is (= {:width 4 :height 3 :board {0 :red 4 :yellow 8 :red 6 :yellow} :game-log []} (sut/place-counter test-4x3-laid-board 2 1 :yellow)))
  )
  (testing "Replaces the piece in position"
    (is (= {:width 4 :height 3 :board {0 :yellow 4 :yellow 8 :red} :game-log []} (sut/place-counter test-4x3-laid-board 0 0 :yellow)))
    (is (= {:width 4 :height 3 :board {0 :red 4 :red 8 :red} :game-log []} (sut/place-counter test-4x3-laid-board 4 0 :red)))
  )
  (testing "Removes the piece if empty selected"
    (is (= {:width 4 :height 3 :board {4 :yellow 8 :red} :game-log []} (sut/place-counter test-4x3-laid-board 0 0 :empty)))
    (is (= {:width 4 :height 3 :board {0 :red 8 :red} :game-log []} (sut/place-counter test-4x3-laid-board 4 0 :empty)))
  )
  )
...
(deftest test-apply-move
  ...
  (testing "apply move returns map with board set to the updated board if move is valid"
    (is (= {:width 4 :height 3 :board {0 :red 1 :red 4 :yellow 8 :red} :game-log [1]} (:board (sut/apply-move test-4x3-laid-board 1 :red))))
  )
...
(deftest test-apply-moves
  ...
  (testing "apply-moves will apply the provided counter first"
    (is (= {:width 6 :height 5 :board {2 :yellow} :game-log [2]} (sut/apply-moves test-6x5-empty-board [2] :yellow)))
    (is (= {:width 6 :height 5 :board {2 :red} :game-log [2]} (sut/apply-moves test-6x5-empty-board [2] :red)))
  )
  (testing "apply-moves will apply the moves flipping counter each turn"
    (is (= {:width 6 :height 5 :board {2 :yellow 3 :red} :game-log [2 3]} (sut/apply-moves test-6x5-empty-board [2 3] :yellow)))
    (is (= {:width 6 :height 5 :board {2 :red 3 :yellow} :game-log [2 3]} (sut/apply-moves test-6x5-empty-board [2 3] :red)))
    (is (= {:width 6 :height 5 :board {1 :yellow 2 :red} :game-log [2 1]} (sut/apply-moves test-6x5-empty-board [2 1] :red)))
    (is (= {:width 6 :height 5 :board {1 :yellow 7 :red} :game-log [1 1]} (sut/apply-moves test-6x5-empty-board [1 1] :yellow)))
    (is (= {:width 6 :height 5 :board {1 :red 7 :yellow} :game-log [1 1]} (sut/apply-moves test-6x5-empty-board [1 1] :red)))
    (is (= {:width 6 :height 5 :board {1 :red 7 :yellow 13 :red} :game-log [1 1 1]} (sut/apply-moves test-6x5-empty-board [1 1 1] :red)))
  )
  )
